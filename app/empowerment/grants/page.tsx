...
export default function GrantsEnterprisePage() {
  const grantProjects = explorerProjects.filter((project) => {
    const text =
    ...
    return (
      ...
      text.includes("economic empowerment")
    );
  });

  const womenSupport = grantProjects.filter((project) => {
    const text =
  ...
  const youthEnterprise = grantProjects.filter((project) => {
    const text =
      `${project.title} ${project.description} ${project.category}`.toLowerCase();
  ...
  const agricultureSupport = grantProjects.filter((project) => {
    const text =
      `${project.title} ${project.description} ${project.category}`.toLowerCase();
  ...
  const businessSupport = grantProjects.filter((project) => {
    const text =
      `${project.title} ${project.description}`.toLowerCase();
  ...
  const categories = [
    {
      title: "Women Enterprise Support",
      value: womenSupport.length,
      icon: HandHeart,
      description:
        "Financial assistance, business starter packs and enterprise-support interventions targeted at women.",
    },
    {
      title: "Youth Enterprise Support",
      value: youthEnterprise.length,
      icon: BriefcaseBusiness,
      description:
        "Entrepreneurship, start-up support and livelihood interventions targeted at young people.",
    },
    {
      title: "Agricultural Support",
      value: agricultureSupport.length,
      icon: Sprout,
      description:
        "Agricultural grants, equipment, livestock and value-chain support for farmers and agribusiness participants.",
    },
    {
      title: "Business Starter Support",
      value: businessSupport.length,
      icon: Store,
      description:
        "Starter kits, equipment and support intended to strengthen small and micro enterprises.",
    },
  ];
      ...
      <main>
          ...
          <Container>
            <div className="relative z-10 max-w-4xl">
              ...
              <h1 className="mt-6 text-4xl font-extrabold leading-tight sm:text-5xl lg:text-6xl">
                Supporting enterprise, livelihoods and economic opportunity
              </h1>

              <p className="mt-6 max-w-3xl text-base leading-8 text-green-50/75 sm:text-lg">
                Explore constituency interventions focused on grants,
                entrepreneurship, business starter support, agricultural
                assistance and livelihood development across Benue South.
              </p>
          ...
          <Container>
            <div className="max-w-3xl">
              ...
              <h2 className="mt-3 text-3xl font-extrabold text-slate-950 sm:text-4xl">
                Support by intervention area
              </h2>

              <p className="mt-4 text-base leading-7 text-slate-600">
                These categories are derived from grant, enterprise and
                livelihood-support records currently available in the
        ...
        >
          <Container>
            <div className="max-w-3xl">
              <p className="text-sm font-bold uppercase tracking-[0.15em] text-green-700">
                Grants & Enterprise Register
              </p>

              <h2 className="mt-3 text-3xl font-extrabold text-slate-950 sm:text-4xl">
                Recorded enterprise and livelihood interventions
              </h2>

              <p className="mt-4 text-base leading-7 text-slate-600">
                Browse constituency records containing grants, enterprise
                support, starter assistance and livelihood interventions.
              </p>
            </div>
            ...
            {grantProjects.length > 0 ? (
              ...
              <div className="mt-8 rounded-[24px] border border-dashed border-slate-300 bg-white px-6 py-14 text-center">
                ...
                <h3 className="mt-4 text-xl font-extrabold text-slate-900">
                  No grant-specific records currently available
                </h3>

                <p className="mt-2 text-sm text-slate-500">
                  Additional grant and enterprise-support records will appear
                  here as the constituency project register is enriched.
                </p>
              </div>
            )}
